...
| 6. Cryptography & Quantum Frontiers | Rotor machines, modular arithmetic, interactive proofs, quantum search and periods | Reason about secrecy, hardness, proof, and quantum speedups | Connect RSA's factoring assumption to Shor's period-finding attack, explain how repeated random challenges make a zero-knowledge proof sound, and why Grover's search scales as the square root of the input |
...
- Module 6: apply number theory to secrecy, prove a fact while revealing nothing, search with quantum interference, then see where quantum computing breaks the hardness.

## Student Rhythm

For each notebook:

1. Run the provided example once.
2. Read the output before reading more explanation.
